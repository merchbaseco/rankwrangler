...
- CI and production database safety: `docs/operations/ci-troubleshooting.md`,
  `docs/operations/database-queries.md`. The Quality workflow's `Check` job runs
  `bun run check:fast` and nothing heavier, deliberately, with database-backed
  tests in a parallel `Database` job; read "Quality is the fast lane, on
  purpose" in `docs/operations/development.md` before changing
