 ...
 */
const containerName = "rankwrangler-server";
...
for (const line of schema.split("\n")) {
    ...
    if (item) {
        const name = item[1];
        const internal = decorators.includes("@internal");
        if (!internal) {
            declared.add(name);
...
for (const name of [...requiredInProduction].sort()) {
    if (!deliveredNames.includes(name)) {
        failures.push(
            `Production-required sensitive item ${name} never reached the container.`
        );
    }
}
...
const deliveredSensitive = deliveredNames.filter((name) =>
    sensitive.has(name)
).length;
