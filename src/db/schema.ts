...
export const displayGroups = pgTable(
    ...
    table => ({
        ...
        categoryLinkIdx: uniqueIndex('display_groups_category_link_idx').on(
            table.category,
            table.link
        ),
    })
...
export const productRankHistory = pgTable(
    'product_rank_history',
    {
        ...
        displayGroupId: uuid('display_group_id')
            .notNull()
            .references(() => displayGroups.id),
        date: date('date').notNull(),
        bsr: integer('bsr').notNull(),
    },
    table => ({
        // Unique index - one rank per product/display_group/date
        productDisplayGroupDateIdx: uniqueIndex(
            'product_rank_history_product_display_group_date_idx'
