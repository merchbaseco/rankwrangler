			...
			<Frame>
				<div className="flex items-center justify-between border-b border-border px-3 py-2">
					<div>
						...
						<p className="text-muted-foreground mt-0.5 text-xs">Admin runtime history and logs</p>
					</div>
				...
				{query.isLoading ? (
					<p className="text-muted-foreground px-3 py-3 text-sm">Loading recent executions...</p>
				) : null}
				...
				{query.error && errorCode !== 'UNAUTHORIZED' && errorCode !== 'FORBIDDEN' ? (
					<p className="text-destructive px-3 py-3 text-sm">Failed to load job executions.</p>
				) : null}
				{!query.isLoading && !query.error && (query.data?.length ?? 0) === 0 ? (
					<p className="text-muted-foreground px-3 py-3 text-sm">No recent execution logs recorded.</p>
				) : null}
