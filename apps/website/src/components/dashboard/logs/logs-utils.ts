...
export type EventLogRow = RouterOutputs['api']['app']['eventLogs']['items'][number];
...
export const statusOptions: EventLogStatus[] = ['success', 'failed', 'pending', 'retrying', 'partial'];
export const typeOptions: EventLogPrimitiveType[] = ['product', 'history', 'job', 'system'];

export const levelBadgeVariantByLevel: Record<EventLogLevel, 'info' | 'warning' | 'error' | 'outline'> = {
    info: 'info',
    warn: 'warning',
    error: 'error',
    debug: 'outline',
...
export const toggleFilter = <T extends string>(current: readonly T[], value: T) => {
