...
export const sliderToBsr = (position: number): number => {
	const bsr = Math.round(
		Math.exp(LOG_MIN + (position / 100) * (LOG_MAX - LOG_MIN)),
	);
...
export const formatBsr = (bsr: number): string => {
	if (bsr >= 1_000_000)
		return `${(bsr / 1_000_000).toFixed(bsr % 1_000_000 === 0 ? 0 : 1)}M`;
	if (bsr >= 1_000)
		return `${(bsr / 1_000).toFixed(bsr % 1_000 === 0 ? 0 : 1)}K`;
	return String(bsr);
};
...
export const FACET_CATEGORY_META: Record<
	string,
