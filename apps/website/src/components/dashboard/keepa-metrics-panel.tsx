...
const KEEPA_METRIC_TOOLTIPS: Record<AdminStatLabel, string> = {
	"Keepa Fetches":
		"Number of Keepa API call attempts (from keepa import rows).",
	"Job Successes":
...
const MINI_STAT_TOOLTIPS: Record<MiniStatLabel, string> = {
	...
	"Auto Refresh":
		"Merch products currently eligible for automatic Keepa refresh.",
	"Merch Products": "Total unique merch products in the products table.",
	"With Keepa":
		"Unique merch products with at least one successful Keepa import.",
	"Without Keepa":
		"Unique merch products that have never had a successful Keepa import.",
};
...
export const KeepaMetricsPanel = () => {
	...
	const keepaRefreshPolicyBuckets =
		statsQuery.data?.keepaRefreshPolicyBuckets ?? [];
		...
		<div className="flex h-full flex-col">
			...
			<div className="grid grid-cols-5 border-t border-border">
				<MiniStat
					...
					value={
						keepaQueueLength === null ? "—" : formatNumber(keepaQueueLength)
					}
				/>
				<MiniStat
					label="Auto Refresh"
				...
				<MiniStat
					...
					value={formatNumber(keepaMerchCoverage.totalMerchProducts)}
				/>
				<MiniStat
					label="With Keepa"
					tooltip={MINI_STAT_TOOLTIPS["With Keepa"]}
					value={formatNumber(keepaMerchCoverage.merchProductsWithKeepaData)}
					type="success"
				/>
				<MiniStat
					label="Without Keepa"
					tooltip={MINI_STAT_TOOLTIPS["Without Keepa"]}
					value={formatNumber(keepaMerchCoverage.merchProductsWithoutKeepaData)}
					type="error"
				/>
			</div>
...
const MetricLabel = ({
	...
	<div className="flex items-center gap-1 text-[10px] font-medium uppercase tracking-wider text-muted-foreground">
		...
		<Tooltip>
			...
			<TooltipPopup className="max-w-[220px] text-balance">
				{tooltip}
			</TooltipPopup>
		</Tooltip>
	</div>
