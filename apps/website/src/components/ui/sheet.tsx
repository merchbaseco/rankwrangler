    ...
    'fixed z-50 flex max-h-screen flex-col overflow-hidden bg-background shadow-xl outline-none transition-transform duration-200 ease-out',
    {
        ...
        compoundVariants: [
            ...
            },
            {
                ...
                variant: 'inset',
                className:
                    'top-2 rounded-2xl border border-border sm:top-4',
            },
        ],
    }
);
...
const SheetPopup = React.forwardRef<
    HTMLDivElement,
    React.ComponentProps<typeof Dialog.Popup> & VariantProps<typeof sheetPopupVariants>
...
>(({ className, ...props }, ref) => (
    <Dialog.Title ref={ref} className={cn('font-semibold text-foreground', className)} {...props} />
));
